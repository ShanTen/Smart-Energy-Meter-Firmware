### VERSION 0005 ***DO NOT REMOVE THIS LINE***
...
def changeLCD():
    ...
    if(display_index == 0):
        ...
        lcd.clear()
        lcd.setCursor(2, 0)
        lcd.printout("Smart Peter")
        lcd.setCursor(4, 1)
        lcd.printout("Ver 0005")
...
while(True):
    ...
    if(int(currentdate) > int(date)):
        ...
        with open('config.json', 'rt') as file:
            data = json.load(file)
        forceflag = int(data["forceUpdate"])
        ...
        if int(currentdate[6:8])%7==0 and updateflag==0:
            ...
            forceflag = 0
            with open ('config.json', 'rt') as read:
                data = json.load(read)
            with open('config.json', 'wt') as write:
                data['forceUpdate'] = 0
                json.dump(data,write)
            cleanup_resources()
            raise NewUpdateTime('forcing an update')
